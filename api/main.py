...
@app.post('/reset')
def reset_default():
    """Reset environment and return observation. Defaults to task_1."""
    obs = envs['task_1'].reset()
    return {"observation": obs.model_dump()}


@app.post('/reset/')
def reset_default_slash():
    """Slash-terminated alias for reset default."""
    obs = envs['task_1'].reset()
    return {"observation": obs.model_dump()}
...
@app.post('/reset/{task_id}/')
def reset_task_slash(task_id: str):
    if task_id not in envs:
        raise HTTPException(status_code=404, detail=f'Task {task_id} not found')
    obs = envs[task_id].reset()
    return {"observation": obs.model_dump()}
