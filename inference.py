...
ENV_BENCHMARK = os.environ.get('ENV_BENCHMARK', 'email-triage-env')
MAX_STEPS = 15
TEMPERATURE = 0.0
...
def emit_step(step_num: int, action: dict, reward: float, done: bool, error) -> None:
    print(
        f'[STEP] step={step_num} action={format_action(action)} '
        f'reward={float(reward):.2f} done={format_bool(done)} error={format_error(error)}'
    )
...
def run_task(task_id: str) -> float:
    ...
    try:
        ...
        for step_num in range(1, MAX_STEPS + 1):
            ...
            if task_id == 'task_1':
                TASK1_AGENT.observe(current_email, sent_action, reward_data)
