...
function getApiBaseUrl() {
  const configured = window.__API_BASE_URL__ || window.API_BASE_URL || "";
...
function apiUrl(path) {
  ...
  return `${getApiBaseUrl()}${cleanPath.startsWith("/") ? "" : "/"}${cleanPath}`;
}
...
async function loadTrainingLogs() {
  const resp = await apiFetch("/training/logs?limit=120");
  if (!resp.ok) {
...
async function runEpochTraining() {
  ...
  const resp = await apiFetch("/training/run", {
    method: "POST",
...
async function loadSupportStats() {
  const resp = await apiFetch("/support/stats");
  if (!resp.ok) {
    throw new Error(`HTTP ${resp.status}`);
  }
  const payload = await resp.json();
...
async function loadAgentStatus() {
  const resp = await apiFetch("/agent/task_1");
  if (!resp.ok) {
    throw new Error(`HTTP ${resp.status}`);
  }
  const payload = await resp.json();
...
async function runSupportSearch() {
  ...
  const resp = await apiFetch("/support/search", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
async function apiReset() {
  const resp = await apiFetch(`/reset/${state.taskId}`, { method: "POST" });
  if (!resp.ok) {
...
async function apiStep(action) {
  const resp = await apiFetch(`/step/${state.taskId}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
async function apiAutoStep() {
  const resp = await apiFetch(`/auto-step/${state.taskId}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
